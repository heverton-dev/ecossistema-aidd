...
def resolve_canonical_destination(
    tipo: str,
    nome: str,
    alvo_projeto: str = "aidd-enterprise",
    ecossistema_root: Optional[Path] = None,
) -> Optional[Path]:
    """Resolve o caminho canônico do componente no monorepo."""
    if tipo == "hook":
        if alvo_projeto == "aidd-master":
            template = "componentes/{alvo_projeto}/hooks/{nome}/hook.sh"
        else:
            template = "componentes/{alvo_projeto}/hooks/{nome}/{nome}.json"
    else:
        return None
    if ecossistema_root is None:
        ecossistema_root = _default_ecossistema_root()
    return Path(ecossistema_root) / template.format(nome=nome, alvo_projeto=alvo_projeto)


def sincronizar_componente(
    tipo: str,
    ferramenta: str = "aidd-enterprise",
    ecossistema_root: Optional[Path] = None,
) -> int:
...
def remover_componente(tipo: str, nome: str, root_dir: str = ".") -> Result:
    ...
    alvo_projeto = componente.get("alvo_projeto", "aidd-enterprise")
...
def materializar(
    ...
    try:
        ...
        alvo_projeto = payload.get("alvo_projeto", "aidd-enterprise")
