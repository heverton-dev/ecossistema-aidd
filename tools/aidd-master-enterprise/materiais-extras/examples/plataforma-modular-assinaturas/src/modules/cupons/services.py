...
class CuponsService:
    ...
    def criar(self, titulo: str, dados: dict = None):
        ...
        with self.db.get_connection() as conn:
            cur = conn.execute(
                "INSERT INTO mod_cupons (titulo, dados_json) VALUES (?, ?)",
                (titulo.strip(), json.dumps(dados_dict))
            )
            conn.commit()
            novo_id = cur.lastrowid
            if self.events:
                self.events.emit("cupons_criado", {"id": novo_id, "titulo": titulo})
            return {"sucesso": True, "id": novo_id, "titulo": titulo}

    def deletar(self, item_id: int):
        with self.db.get_connection() as conn:
            conn.execute("DELETE FROM mod_cupons WHERE id = ?", (item_id,))
            conn.commit()
            if self.events:
                self.events.emit("cupons_deletado", {"id": item_id})
            return {"sucesso": True}
