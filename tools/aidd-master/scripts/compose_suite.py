...
__INIT_SCHEMAS__
    ...
    if cur.fetchone()[0] == 0:
        conn.execute("INSERT INTO webhooks (url, secret, eventos, ativo) VALUES ('https://webhook.site/demo-aidd-master', 'sec_demo_2026', '*', 1)")
    cur.execute("SELECT count(*) FROM webhook_logs")
    if cur.fetchone()[0] == 0:
        conn.execute("INSERT INTO webhook_logs (webhook_id, evento, url, payload_json, status_code, resposta, sucesso) VALUES (1, 'suite.criado', 'https://webhook.site/demo-aidd-master', '{\\\"event\\\":\\\"suite.criado\\\",\\\"data\\\":{\\\"id\\\":1}}', 200, '{\\\"received\\\": true}', 1)")
    conn.commit()
