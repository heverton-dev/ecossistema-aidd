...
@cli.command("inject", help="Injeta e integra um novo componente (skill/mcp/rule/spec/config/agent/hook)")
@click.argument("tipo", type=click.Choice(["skill", "mcp", "rule", "spec", "config", "agent", "hook"]))
@click.argument("nome")
@click.option("--descricao", "-d", default="", help="Descrição funcional do componente em PT-BR")
@click.option("--conteudo-file", "conteudo_file", default=None, help="Arquivo com o conteúdo completo do artefato (senão, gera scaffold padrão completo)")
@click.option("--mcp-command", "mcp_command", default=None, help="Comando executável para servidor MCP externo (registra em mcp.json)")
@click.option("--mcp-args", "mcp_args", default=None, help='Lista JSON de argumentos para o servidor MCP externo, ex: \'["--flag"]\'')
@click.option("--mcp-env", "mcp_env", default=None, help='Objeto JSON de variáveis de ambiente para o servidor MCP externo, ex: \'{"KEY": "VAL"}\'')
@click.option("--projeto", default="aidd-master", help="Projeto alvo com perfil resolvido (default: aidd-master)")
@click.option("--sobrescrever", is_flag=True, default=False, help="Sobrescreve destinos já existentes em disco")
@click.option("--remover", is_flag=True, default=False, help="Remove um componente previamente injetado")
@click.option("--dry-run", "dry_run", is_flag=True, default=False, help="Simula a injeção sem escrever arquivos no disco")
@click.option("--dir", default=".", help="Diretório raiz do projeto (default: diretório atual)")
def _inject_cmd(tipo, nome, descricao, conteudo_file, mcp_command, mcp_args, mcp_env, projeto, sobrescrever, remover, dry_run, dir):
    cmd_inject(types.SimpleNamespace(
        tipo=tipo, nome=nome, descricao=descricao, conteudo_file=conteudo_file,
        mcp_command=mcp_command, mcp_args=mcp_args, mcp_env=mcp_env, projeto=projeto,
        sobrescrever=sobrescrever, remover=remover, dry_run=dry_run, dir=dir,
    ))
