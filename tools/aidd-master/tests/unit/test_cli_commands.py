# -*- coding: utf-8 -*-
"""
Testes automatizados dos comandos CLI do AIDD Master (scripts/aidd.py):
- cmd_audit
- cmd_plan
- cmd_apply
- cmd_compose_orca
...
def test_cmd_audit_sucesso_todos_gates_passam(suite_composta):
    """1.3: Executa cmd_audit contra projeto composto e confirma exit 0 e todos PASS."""
    proc = subprocess.run(
...
def test_cmd_audit_falha_deliberada_especifica_g_estrutura(suite_composta):
    """1.4: Remove PLANO-EXECUCAO-ESTRUTURADO.json e confirma exit 1 com G_ESTRUTURA FAIL."""
...
def test_cmd_apply_executa_plano_com_sucesso(tmp_path):
    """2.2: cmd_apply lê PLANO-EXECUCAO-ESTRUTURADO.json e executa compose_suite."""
    import argparse
...
def test_cmd_apply_falha_quando_manifesto_ausente(tmp_path):
    """2.2 (falha): cmd_apply retorna exit 1 quando manifesto não existe."""
    proc = subprocess.run(
...
def test_cmd_compose_orca_sucesso_gera_estrutura_e_manifesto(tmp_path):
    """3.1 e 3.2: cmd_compose_orca gera arquivos padrão por módulo e manifesto com status APROVADO."""
...
def test_cmd_compose_orca_falha_retorna_exit_1(tmp_path):
    """3.3: Força falha em módulo (nome com aspas gerando erro sintático no script worker) e valida exit 1."""
...
def test_cmd_bench_sucesso_concorrencia_wal(suite_composta):
    """4.1 a 4.3: Executa cmd_bench contra projeto composto, exit 0, zero falhas e contagem exata."""
    num_operacoes = 60
    proc = subprocess.run(
...
def test_cmd_bench_falha_quando_banco_inacessivel(tmp_path):
    """4.2 (falha): cmd_bench retorna exit 1 quando o diretório impede criação do app.db."""
...
def test_cmd_export_frontend_sucesso_gera_arquivos_ts(suite_composta):
    """5.1 e 5.2: Executa cmd_export_frontend contra suite composta e confirma geracao dos arquivos .ts/.tsx."""
    proc = subprocess.run(
...
def test_cmd_export_frontend_validacao_sintatica_estrutural_ts(suite_composta):
    """5.3: Valida sintaxe e integridade estrutural do TypeScript gerado."""
    proc = subprocess.run(
        [sys.executable, AIDD_SCRIPT, "export-frontend", "--dir", str(suite_composta)],
        capture_output=True,
        text=True,
...
def test_cmd_export_frontend_stack_invalida_lanca_erro(suite_composta):
    """5.2/5.3: Valida rejeição explícita de stack diferente de nextjs."""
...
def test_cmd_refine_module_executa_behave_cenario_real_sucesso(suite_composta):
    """6.1 e 6.2: Executa cmd_refine_module com behave real e confirma cenarios aprovados."""
...
def test_cmd_refine_module_falha_quando_cenario_bdd_falha(suite_composta):
    """6.2 (falha): Cenário BDD com asserção com falha retorna exit code diferente de zero."""
    features_dir = suite_composta / "features"
    steps_dir = features_dir / "steps"
    steps_dir.mkdir(parents=True, exist_ok=True)
...
def test_cmd_refine_module_falha_quando_spec_nao_encontrado(suite_composta):
    """6.2 (spec ausente): Retorna exit 1 quando o arquivo .feature não existe."""
    proc = subprocess.run(
...
def test_cmd_refine_module_estrutural_argumentos_behave(tmp_path, monkeypatch):
    """6.3: Confirma que cmd_refine_module monta argumentos e ambiente corretos para invocar behave."""
    import argparse
...
def test_cmd_setup_diagnostico_completo_com_pip_isolado(monkeypatch, capsys):
    """7.1 e 7.2: Testa a parte diagnóstica de cmd_setup isolando o pip install para não mutar o ambiente."""
    import argparse
...
def test_cmd_setup_tolerante_a_aviso_instalacao(monkeypatch, capsys):
    """7.2: Confirma que cmd_setup emite [WARN] e não retorna código de falha quando instalação falha."""
    import argparse
    from aidd import cmd_setup
