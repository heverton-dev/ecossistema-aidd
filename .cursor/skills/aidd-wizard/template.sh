...
# Generated by the /aidd-wizard skill.
# Adapted from wizard/template.sh in mattpocock/skills (commit c55ee46), MIT license.
# AIDD change: open_url supports Git Bash on Windows (cmd.exe start).
#
...
# open_url URL opens it in the human's browser, cross-platform incl. WSL and Git Bash.
open_url() {
  local url="$1"
  printf '  %s↗ opening%s %s\n' "$GREEN" "$RESET" "$url"
  { if   command -v wslview     >/dev/null 2>&1; then wslview "$url"
    elif [[ "${OSTYPE:-}" == msys* || "${OSTYPE:-}" == cygwin* ]] && command -v cmd.exe >/dev/null 2>&1; then
      cmd.exe //c start "" "$url"   # Git Bash on Windows; // stops MSYS path conversion
