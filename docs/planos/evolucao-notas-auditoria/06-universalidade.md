...
> **Atualização (Rodada 2, 05/09/2026):** o mecanismo do `agy` deixou de ser "desconhecido" — identificado por investigação estática de custo zero (nenhuma chamada de LLM gasta), sem reabrir este pacote. Ver `docs/planos/refinamento-notas-auditoria/05-investigacao-mecanismo-agy.md`: `agy` descobre skills em `.agents/skills/` (plural), não em `.agent/skills/` (o que este ecossistema gera). Uma contradição real com a documentação oficial do `agy` (que alega suporte retroativo ao formato singular) permanece registrada como não resolvida — por decisão explícita do usuário de não gastar uma chamada real adicional. `freebuff` continua sem mecanismo confirmado, sem novidade.
