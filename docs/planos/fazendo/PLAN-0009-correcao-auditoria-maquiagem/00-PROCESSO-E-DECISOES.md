...
| # | Item | Documento |
|---|---|---|
| 1 | Gates rodarem pytest de verdade (nao so estrutura) | `01-gates-rodarem-pytest.md` |
| 2 | Telemetria de testes sempre remedida, nunca gravada estatica | `02-telemetria-testes-sempre.md` |
| 3 | G_ZERO_HEADLESS: enforcement real, nao so grep de string | `03-g-zero-headless.md` |
| 4 | Reverter CSP relaxado no template compartilhado (unsafe-inline/CDN) | `04-reverter-csp-relaxado.md` |
| 5 | Remover seed de demo (webhook.site + secret hardcoded) da geracao padrao | `05-remover-seed-demo.md` |
| 6 | enterprise inject: dry-run real por padrao antes de gravar no working tree | `06-enterprise-inject-dry.md` |
| 7 | Corrigir gate de seguranca rotulado blindagem militar (rotulo ou cobertura real) | `07-corrigir-gate-seguranca.md` |
| 8 | Sincronizar CLI do aidd-ops com a suite de testes | `08-sincronizar-cli-aidd.md` |
| 9 | Corrigir porta duplicada no compose gerado e parar dashboard de fabricar resultado de preflight | `09-corrigir-porta-duplicada.md` |
| 10 | Corrigir CORS inseguro (allow_origins=* + allow_credentials=True) no app gerado pelo generator | `10-corrigir-cors-inseguro.md` |
| 11 | Corrigir deploy Docker do modulo gerado (pip install ausente, nginx/ inexistente, secret em texto plano) | `11-corrigir-deploy-docker.md` |
| 12 | Prova real de integracao multi-ferramenta no teste integrado | `12-prova-real-integracao.md` |
| 13 | Reduzir custo de token nas ferramentas de nucleo deterministico (master/enterprise/ops/CLI) | `13-reduzir-custo-token.md` |
| 14 | Remover ou ligar codigo morto de resiliencia no enterprise | `14-remover-ou-ligar.md` |
| 15 | requirements.txt do app gerado pelo generator dessincronizado do codigo real | `15-requirementstxt-app-gerado.md` |
| 16 | XSS armazenado real em `get_studio_html` (núcleo compartilhado) | `16-xss-armazenado-real.md` |
| 17 | CLI aidd_inject convertida de argparse para click pela metade | `17-cli-aidd-inject.md` |
| 18 | CLI do aidd-forge convertida de argparse para click pela metade (suíte quebrada) | `18-cli-forge-argparse.md` |
