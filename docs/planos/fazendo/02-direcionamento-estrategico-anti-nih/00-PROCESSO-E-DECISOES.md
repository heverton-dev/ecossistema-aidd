...
| 6 | Fase 4 - Investimento no diferencial real (pipeline do generator, protocolo delegado, materializador multi-harness) | 🔄 Definição concluída (2026-09-07). Execução aprovada em 2026-09-08 (auditoria + correção nesta sessão, ver §1.1 do `06`): **6.1 ✅, 6.3 ✅, 6.4 ✅, 6.5 ✅ concluídos e reproduzidos** (testes reais passando); **6.2 ⏳ não iniciado**, segue sem aprovação | `06-fase-4---investimento-no-diferencial-real-pipeline-do-generator-protocolo-delegado-materializador-multi-harness.md` |
...
|---|---|---|---|
| 1 | `fleet_discovery.py` continua byte-identico entre `tools/aidd-enterprise/src/core/fleet_discovery.py` e `tools/aidd-master/src/core/fleet_discovery.py` (367 linhas cada, confirmado por diff). A correcao do item 6.5 (Kiro CLI, Hermes, resolucao de extensao Windows) foi aplicada manualmente 3x (generator, enterprise, master) em vez de consolidada num modulo compartilhado — reforca exatamente o padrao de duplicacao que esta iniciativa anti-NIH existe para eliminar. Efeito colateral concreto ja observado: rodar os 3 arquivos de teste de fleet discovery juntos num unico `pytest` a partir da raiz do monorepo falha por colisao de nome de modulo (`tests.unit.test_fleet_discovery` duplicado entre enterprise e master); rodados separadamente, os 3 passam (105/105 testes). | Auditoria da Fase 4 (2026-09-08), achado 6 | ⏳ Registrado, aguardando decisao explicita do usuario sobre consolidar (e onde) ou aceitar como duplicacao intencional |
