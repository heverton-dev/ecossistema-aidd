...
- **Definição de Pronto (DpP):**
  ...
  - **Evidência de validação (reproduzida em 2026-09-08):** `pytest` real por ferramenta — `aidd-enterprise`: 36/36 passed; `aidd-master`: 36/36 passed; `aidd-generator`: 34/34 passed (105 testes reais no total). Nota lateral registrada: `fleet_discovery.py` de `aidd-enterprise` e `aidd-master` continua byte-idêntico entre si (duplicação pré-existente, reforçada aqui em vez de eliminada) — ver achado registrado em `00-PROCESSO-E-DECISOES.md` como débito técnico a decidir, não escondido.
