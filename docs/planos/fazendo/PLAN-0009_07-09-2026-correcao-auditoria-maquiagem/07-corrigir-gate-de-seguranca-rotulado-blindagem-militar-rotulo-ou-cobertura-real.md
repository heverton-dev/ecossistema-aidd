...
> **Status:** [RASCUNHO — Aguardando Aprovacao Humana. **NOTA 2026-09-09 (sessão de fechamento anti-NIH, executor Buffy):** a correção foi aplicada de fato nos 4 arquivos do par (`scripts/gates/` + `templates/gates/`, master e enterprise, idênticos — verificado por drift gate). Rota tomada: **rótulo honesto (B) + os 2 checks comportamentais que os 4 testes de regressão já commitados em `test_g_seguranca_gate.py` exigem** — Camada 3b (SQLi: bind parameter neutraliza payload real em sqlite3, com prova da pré-condição de vazamento por concatenação) e Camada 3c (XSS: payload real injetado no catálogo e renderizado pelo mesmo `get_studio_html` da rota /webhooks, com escape verificado); relatório final sem linguagem de marketing e reportando composição da cobertura (comportamentais/configuração/estáticos). Prova: 4/4 testes passando em ambas as ferramentas; `G_HONESTIDADE_ROTULO.py` APROVADO (51 arquivos, 0 termos) — primeira aprovação desde a criação do gate; drift gate APROVADO. A decisão formal A-vs-B sobre INVESTIR em mais cobertura comportamental além destes 2 checks continua em aberto para o humano; o rótulo, porém, já é fiel à cobertura real executada.]
