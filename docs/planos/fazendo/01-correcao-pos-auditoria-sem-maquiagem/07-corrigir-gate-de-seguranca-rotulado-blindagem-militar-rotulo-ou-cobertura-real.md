...
> **Status:** [RASCUNHO — Aguardando Aprovacao Humana — rota depende de decisão do usuário. NOTA 2026-09-09 (sessão de retomada): os 4 testes de regressão deste item (`tests/unit/test_g_seguranca_gate.py`, master e enterprise) existem na árvore, mas a correção do `scripts/gates/G_SEGURANCA.py` foi revertida junto com outras mudanças não commitadas por sessão concorrente (plano `codigo-limpo-profundo-ecossistema`, cujo item 16 decompondo a god class SecurityGate está em andamento neste exato arquivo). NÃO re-aplicado aqui para evitar colisão de edição concorrente; os 2 testes do gate voltam a falhar até a decisão humana do rótulo/cobertura pousar em uma das duas frentes.]
