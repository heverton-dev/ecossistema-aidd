...
> **Atualização (Rodada 2, 05/09/2026):** o tipo `hook` de ambos os injetores (`aidd-master` e, desde o Item 4 desta rodada, `aidd-enterprise` também) passou a gravar uma cópia canônica adicional em `componentes/{alvo_projeto}/hooks/{nome}/...` e chamar `gestor_componentes.py sync`, como passo extra ao lado do fan-out direto já existente — não uma migração completa dos injetores para o mecanismo `componentes/`, mas fecha a divergência específica do tipo `hook` citada aqui. Ver `docs/planos/refinamento-notas-auditoria/04-unificacao-injetor-aidd-enterprise.md`.
