...
| # | Item | Documento |
|---|---|---|
| 1 | Intake Interativo Web Sem Friccao | `01-intake-interativo-web.md` |
| 2 | Cofre Local e Coleta Segura de Credenciais | `02-cofre-local-coleta.md` |
| 3 | AppShell White-Label e Studios OpenAPI Webhook MCP | `03-appshell-white-label.md` |
| 4 | Isolamento Estrito em VPS Compartilhada e Uninstall Atomico | `04-isolamento-estrito-vps.md` |
...
|---|---|---|---|
| 1 | Intake Interativo Web Sem Friccao | ✅ Concluído via Coolify + Streamlit (2026-09-07, NIH #19) — `apps/intake/` (Streamlit 8501 headless reusando `montar_plano_em_memoria`) + `Dockerfile.intake` + deploy como app gerenciado via `pipeline_ops.py coolify create/setenv/deploy`; 144 testes aidd-ops verdes, G_OPS_MVP e G_OPS_SSH aprovados | `01-intake-interativo-web.md` |
| 2 | Cofre Local e Coleta Segura de Credenciais | ✅ Concluído via sops+age (2026-09-07, NIH #18/#29) — `CofreCredenciais` (`src/core/cofre_credenciais.py`) + CLI `pipeline_ops.py cofre init/encrypt/decrypt/up`; 17/17 testes reais com binários `sops`/`age-keygen` (`tests/test_cofre_credenciais.py`), reverificados em 2026-09-09. Linha corrigida — estava desatualizada, o item já constava concluído em seu próprio documento desde 2026-09-07 | `02-cofre-local-coleta.md` |
| 3 | AppShell White-Label e Studios OpenAPI Webhook MCP | ✅ Concluído via Coolify (2026-09-07, NIH #20) — Coolify Dashboard adotado como base para AppShell white-label + Studios integrados via `CoolifyManager` | `03-appshell-white-label.md` |
| 4 | Isolamento Estrito em VPS Compartilhada e Uninstall Atomico | ✅ Concluído via Coolify (2026-09-07, NIH #21) — Isolamento nativo de containers em VPS compartilhada implementado via `CoolifyManager` (redes isoladas, zero portas expostas, limites CPU/RAM) | `04-isolamento-estrito-vps.md` |
