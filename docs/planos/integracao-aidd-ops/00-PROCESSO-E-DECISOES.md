...
| 3. MVP `tools/aidd-ops/` (Fases 1-3) | ✅ Concluído em 06/09/2026 — Fases 1 (Intake), 2 (Curadoria) e 3 (Sizing) implementadas de forma 100% determinística (zero LLM), Result monad, 3 JSON Schemas, catálogos em data/, orquestrador pipeline_ops.py, gate próprio G_OPS_MVP aprovado, 21 testes verdes (commit 3ebf29a / e22b53a) | `03-mvp-tools-aidd-ops-fases-1-3.md` |
| 4. SSH Runner | ⏳ **PRÓXIMO NA FILA** — Escrito por agente paralelo, verificado e corrigido por auditoria independente: amarrado ao Result monad/gate próprio do Pacote 3, requirements.txt com paramiko, gate estrutural para lista fechada de operações | `04-ssh-runner-execucao-remota.md` |
| 5. MCPs de borda (Cloudflare/Docker) | ⏳ Aguardando Pacote 4 — Escopo aidd-ops já registrado no manifesto, diretório componentes/aidd-ops/mcps/ pronto para receber cloudflare-mcp e docker-mcp | `05-mcps-borda-cloudflare-docker.md` |
| 6. Gate `G_INFRA_COMPOSE` | ⏳ Escrito por agente paralelo, **verificado e corrigido**: dependência de Docker tratada com honestidade; referência corrigida para PLANO-INFRAESTRUTURA.json; caminho de templates alinhado ao Pacote 8 | `06-gate-infra-compose.md` |
| 7. Comando `ops preflight` (Pre-Flight E2E) | ⏳ Escrito por agente paralelo, **verificado e ajustado**: barreira de aprovação pontual confirmada correta; método de teste com servidor HTTP/TLS local real; subcomando `ops preflight` | `07-gate-preflight-e2e.md` |
| 8. Templates canônicos de infraestrutura | ⏳ Escrito por agente paralelo, **verificado e corrigido**: templates em `tools/aidd-ops/templates/infra/`. Decisão do usuário (06/09/2026): Parte B rejeitada (cada ferramenta é standalone) | `08-templates-canonicos-e-generator-flag.md` |
| 9. Validação E2E com piloto real | ⏳ Escrito por agente paralelo, **verificado e corrigido**: comando `ops deploy <ambiente>` implementado na Fase 0 deste pacote; rollback seguro e confirmação explícita de credenciais | `09-validacao-e2e-piloto-real.md` |

Esta tabela é atualizada ao final de cada pacote — nunca antes da validação real daquele pacote. Os Pacotes 1, 2 e 3 estão concluídos e commitados no repositório. O Pacote 4 é o próximo da fila.
