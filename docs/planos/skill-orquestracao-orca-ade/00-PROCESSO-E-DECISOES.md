...
|---|---|---|---|
| 1 | Núcleo mecânico: `plan_parser.py` + `state_engine.py` + `worktree_engine.py` | Zero | `01-nucleo-mecanico-parser-state-worktree.md` |
| 2 | `gate_auditor.py` integrado ao `ecossistema.py audit` já existente | Zero | `02-gate-auditor-integracao.md` |
| 3 | `harness_profiles.json` (corrigido) + `agent_spawner.py` (compilação de comando, sem disparar LLM real ainda) | Zero (só compila string de comando, não executa) | `03-harness-profiles-agent-spawner.md` |
| 4 | Hooks pre/post + circuit breaker/heartbeat/timeout | Zero | `04-hooks-circuit-breaker.md` |
| 5 | CLI `ecossistema.py orchestrate` + Plano de Voo (`--dry-run`/`--resume`/`--yes`) | Zero (`--dry-run` nunca invoca agente real) | `05-cli-orchestrate-plano-de-voo.md` |
| 6 | Validação end-to-end com agentes reais | **Real e não-trivial** (até 4 CLIs em paralelo) | `06-validacao-end-to-end-agentes-reais.md` (gate de aprovação, sem Definição de Pronto travada ainda) |
