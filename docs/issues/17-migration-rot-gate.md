...
Law #3 (Structured Persistence) is the law at stake, across SQLite WAL and PostgreSQL.
...
- [ ] Migrations applied up and down in an ephemeral database per run; no real database touched.
- [ ] Final schema compared against the declared schema; exit 1 on divergence.
- [ ] Re-applying a migration is proven idempotent, not assumed.
- [ ] Missing rollback scripts cause exit 1.
- [ ] Engine coverage stated explicitly. If only one engine is exercised, output says so (Law #8).
- [ ] Failing-path test, strict Law #13: remove a rollback, execute the gate, assert exit 1.
- [ ] Declared against Law #3 in `AGENTS.md`, per ISSUE-0010 convention.
