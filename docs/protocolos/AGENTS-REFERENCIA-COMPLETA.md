...
- gates/G_USER_FACING_PTBR.py: Audita superfícies lidas pelo usuário leigo (README, GEMINI.md, perfil `preflight-host --perfil leigo`) exigindo sigla traduzida na 1ª ocorrência e zero jargão cru (ISSUE-USA-0004, Rule 10).
...
- gates/G_LAYOUT_ENTREGA.py: Audita o helper `resolve_pasta_entrega` e bloqueia entrega gerada sob `<clone>/projetos/` quando existe projeto legado irmão (ISSUE-USA-0002, Lei #1).
- gates/G_PACOTE_CORE.py: Bloqueia artefato dev-only (`*.db`, `requirements-dev*`, `docs/relatorios/`) no escopo de distribuição do usuário final e exige `export-ignore` no `.gitattributes` (ISSUE-USA-0006, Lei #1).
- gates/G_RESUMO_USUARIO.py: Exige `RESUMO-USUARIO.md` (≤20 linhas, 3 perguntas) e `RELATORIO-TECNICO.md` no fecho de entrega (ISSUE-USA-0007, Lei #1).
- gates/G_ANT_LOCKIN_LEGADO.py: Varredura determinística lovable/supabase/firebase na entrega; allowlist em `lockin-allowlist.txt`; nunca apaga (ISSUE-USA-0008, Lei #1/#7/#8).
