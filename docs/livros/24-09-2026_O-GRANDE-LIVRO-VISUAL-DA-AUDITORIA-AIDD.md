...
subtitle: "A Fábrica de Software Perfeita: Uma Viagem Pelas 8 Ferramentas Macro, 68 Micro-Ferramentas, 50 Guardas e 141 Componentes"
author:
...
abstract: |
  ...
  milimetricamente auditadas). Cobre com precisão cirúrgica as 8 Ferramentas Macro, as 68 Micro-Ferramentas (Skills),
  os 50 Quality Gates determinísticos e os 141 Componentes Agnósticos sincronizados em múltiplos ambientes.
---
...
2. **68 Ferramentas de Precisão no Cinto dos Mestres (As Micro-Ferramentas / Skills):** Lupas, réguas, tesouras mágicas e parafusadeiras que realizam tarefas hiperespecíficas.
3. **50 Guardas Incorruptíveis na Porta (Os Quality Gates):** Inspetores robóticos que não deixam passar nada se tiver um único parafuso torto ou se faltar a etiqueta de segurança.
4. **141 Peças Fundamentais (Os Componentes Agnósticos):** Blocos de encaixe perfeito que funcionam em qualquer mesa de trabalho (seja no Claude, no Gemini, no Cursor, no OpenCode ou no Mimo).
...
# PARTE II: O CINTO DE UTILIDADES (AS 68 MICRO-FERRAMENTAS / SKILLS)

## Na Festa
Imagine o cinto de utilidades do Batman ou a caixa de ferramentas mágica de um relojoeiro suíço. Cada uma dessas 68 ferramentas tem um formato único e resolve exatamente um problema específico sem fazer barulho nem desperdiçar energia.

## Na Casa (O Catálogo Completo das 68 Skills Canônicas)
...
- **As 11 Dimensões desta Micro-Ferramenta:**
  ...
  6. *Scripts 0 LLM:* Nenhum script próprio: a skill é um roteiro em `SKILL.md`.
  7. *Hooks:* Roda automaticamente no commit via `.pre-commit-config.yaml`. Distribuição aos harnesses via `python ecossistema.py components sync --tipo todos`.
...
- **As 11 Dimensões desta Micro-Ferramenta:**
  1. *Recebe (Input):* Chamada de slash command (`/aidd-auditor-4f-runner`) ou diretiva procedural no prompt.
  2. *Cria / Processa:* Roteiro determinístico contido em `SKILL.md` orientando a IA ou script local.
  3. *Entrega (Output):* Artefato estruturado (Markdown, JSON ou diff de código).
  4. *Configs:* Parâmetros no YAML frontmatter (`name`, `description`).
  5. *Gates:* `G_SKILL_ROT.py` confere se todo caminho, script e comando citado no `SKILL.md` existe de verdade.
...
- **As 11 Dimensões desta Micro-Ferramenta:**
  1. *Recebe (Input):* Chamada de slash command (`/aidd-evolucao-runner`) ou diretiva procedural no prompt.
  2. *Cria / Processa:* Roteiro determinístico contido em `SKILL.md` orientando a IA ou script local.
  3. *Entrega (Output):* Artefato estruturado (Markdown, JSON ou diff de código).
  4. *Configs:* Parâmetros no YAML frontmatter (`name`, `description`).
  5. *Gates:* `G_SKILL_ROT.py` confere se todo caminho, script e comando citado no `SKILL.md` existe de verdade.
...
- **As 11 Dimensões desta Micro-Ferramenta:**
  ...
  4. *Configs:* Parâmetros no YAML frontmatter (`name`, `description`).
  5. *Gates:* `G_SKILL_ROT.py` confere se todo caminho, script e comando citado no `SKILL.md` existe de verdade.
  6. *Scripts 0 LLM:* `livro.py` em `scripts/`
  7. *Hooks:* Roda automaticamente no commit via `.pre-commit-config.yaml`. Distribuição aos harnesses via `python ecossistema.py components sync --tipo todos`.
  8. *Agents:* Personas especializadas convocadas conforme o contexto da tarefa.
  9. *Skills:* Integrada ao ecossistema multi-harness.
  10. *MCPs:* Compatível com chamadas de ferramentas de contexto.
...
- **As 11 Dimensões desta Micro-Ferramenta:**
  ...
  4. *Configs:* Parâmetros no YAML frontmatter (`name`, `description`).
  5. *Gates:* `G_SKILL_ROT.py` confere se todo caminho, script e comando citado no `SKILL.md` existe de verdade.
  6. *Scripts 0 LLM:* `analisador.py`, `cli.py`, `fallback.py`, `handoff.py`, `isolamento.py`, `observabilidade.py`, `rollback.py` em `scripts/`
  7. *Hooks:* Roda automaticamente no commit via `.pre-commit-config.yaml`. Distribuição aos harnesses via `python ecossistema.py components sync --tipo todos`.
  8. *Agents:* Personas especializadas convocadas conforme o contexto da tarefa.
  9. *Skills:* Integrada ao ecossistema multi-harness.
  10. *MCPs:* Compatível com chamadas de ferramentas de contexto.
...
### 32. Micro-Ferramenta: `aidd-skills`
...
### 35. Micro-Ferramenta: `aidd-tickets`
...
### 36. Micro-Ferramenta: `cloudflare`
...
### 37. Micro-Ferramenta: `cloudflare-email-service`
...
### 38. Micro-Ferramenta: `cloudflare-one`
...
### 39. Micro-Ferramenta: `cloudflare-one-migrations`
...
### 40. Micro-Ferramenta: `componentes-runner`
...
### 41. Micro-Ferramenta: `debug-issue`
...
### 42. Micro-Ferramenta: `dependencia-runner`
...
### 43. Micro-Ferramenta: `durable-objects`
...
### 44. Micro-Ferramenta: `explore-codebase`
...
### 45. Micro-Ferramenta: `fluxo-01-runner`
...
### 46. Micro-Ferramenta: `fluxo-02-runner`
...
### 47. Micro-Ferramenta: `fluxo-03-runner`
...
### 49. Micro-Ferramenta: `impeccable`
...
### 50. Micro-Ferramenta: `mcp-creator-runner`
...
### 52. Micro-Ferramenta: `nextjs-on-cloudflare`
...
### 54. Micro-Ferramenta: `orca-plan-orchestrator`
...
- **As 11 Dimensões desta Micro-Ferramenta:**
  ...
  4. *Configs:* Parâmetros no YAML frontmatter (`name`, `description`).
  5. *Gates:* `G_SKILL_ROT.py` confere se todo caminho, script e comando citado no `SKILL.md` existe de verdade.
  6. *Scripts 0 LLM:* `agent_spawner.py`, `circuit_breaker.py`, `flight_plan.py`, `gate_auditor.py`, `hooks.py`, `orca_real_plan.py`, `orchestrator_engine.py`, `plan_io.py`, `plan_parser.py`, `state_engine.py`, `subagent_plan.py`, `worktree_engine.py` em `scripts/`
  7. *Hooks:* Roda automaticamente no commit via `.pre-commit-config.yaml`. Distribuição aos harnesses via `python ecossistema.py components sync --tipo todos`.
  8. *Agents:* Personas especializadas convocadas conforme o contexto da tarefa.
  9. *Skills:* Integrada ao ecossistema multi-harness.
  10. *MCPs:* Compatível com chamadas de ferramentas de contexto.
...
### 55. Micro-Ferramenta: `orchestrate`
...
### 57. Micro-Ferramenta: `planos-auditoria-runner`
...
### 59. Micro-Ferramenta: `refactor-safely`
...
### 60. Micro-Ferramenta: `review-changes`
...
### 61. Micro-Ferramenta: `sandbox-migrate-to-next`
...
### 62. Micro-Ferramenta: `sandbox-next`
...
### 63. Micro-Ferramenta: `sandbox-stable`
...
### 64. Micro-Ferramenta: `skill-creator-runner`
...
### 65. Micro-Ferramenta: `turnstile-spin`
...
### 67. Micro-Ferramenta: `workers-best-practices`
...
# PARTE III: OS 50 GUARDIÕES INCORRUPTÍVEIS (QUALITY GATES)

## Na Festa
Imagine 50 cães de guarda robóticos sentados na saída da fábrica. Cada um tem um sensor diferente. Um cheira se tem segredo vazando, outro mede a espessura da parede, outro confere se tem botão quebrado e outro morde o pneu para ver se está furado. Se um único guarda latir (der exit 1), o portão de saída se tranca imediatamente e ninguém sai até consertar!

## Na Casa (A Matriz dos 50 Quality Gates Canônicos)
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  4. *Configs:* Argumentos CLI: `--relatorio`
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Não está no `.pre-commit-config.yaml`: roda sob demanda (`python gates/G_amelhoria.py`).
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  2. *Cria / Processa:* Análise estática por AST Python, regex ou schema JSON (Zero LLM).
  3. *Entrega (Output):* Código binário rigoroso (`exit 0` = APROVADO, `exit 1` = BLOQUEADO).
  4. *Configs:* Sem argumentos de CLI (roda sobre o repositório inteiro).
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Roda automaticamente no commit via `.pre-commit-config.yaml`.
  8. *Agents:* Executa sem intervenção de agente cognitivo.
  9. *Skills:* Invocado pelas skills de runner e auditoria.
  10. *MCPs:* Nenhum — o portão não depende de servidor MCP para rodar.
  11. *Rules:* Subordinado à Lei #2 (Saída Estritamente Binária).
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  2. *Cria / Processa:* Análise estática por AST Python, regex ou schema JSON (Zero LLM).
  3. *Entrega (Output):* Código binário rigoroso (`exit 0` = APROVADO, `exit 1` = BLOQUEADO).
  4. *Configs:* Argumentos CLI: `--base`, `--todos`
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Roda automaticamente no commit via `.pre-commit-config.yaml`.
  8. *Agents:* Executa sem intervenção de agente cognitivo.
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  2. *Cria / Processa:* Análise estática por AST Python, regex ou schema JSON (Zero LLM).
  3. *Entrega (Output):* Código binário rigoroso (`exit 0` = APROVADO, `exit 1` = BLOQUEADO).
  4. *Configs:* Argumentos CLI: `--committed-spec`, `--projeto`, `--url`
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Roda automaticamente no commit via `.pre-commit-config.yaml`.
  8. *Agents:* Executa sem intervenção de agente cognitivo.
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  2. *Cria / Processa:* Análise estática por AST Python, regex ou schema JSON (Zero LLM).
  3. *Entrega (Output):* Código binário rigoroso (`exit 0` = APROVADO, `exit 1` = BLOQUEADO).
  4. *Configs:* Argumentos CLI: `--files`
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Roda automaticamente no commit via `.pre-commit-config.yaml`.
  8. *Agents:* Executa sem intervenção de agente cognitivo.
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  2. *Cria / Processa:* Análise estática por AST Python, regex ou schema JSON (Zero LLM).
  3. *Entrega (Output):* Código binário rigoroso (`exit 0` = APROVADO, `exit 1` = BLOQUEADO).
  4. *Configs:* Argumentos CLI: `--manifesto`
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Não está no `.pre-commit-config.yaml`: roda sob demanda (`python gates/G_DISPATCH_PIPELINE_VSA.py`).
  8. *Agents:* Executa sem intervenção de agente cognitivo.
  9. *Skills:* Invocado pelas skills de runner e auditoria.
  10. *MCPs:* Nenhum — o portão não depende de servidor MCP para rodar.
  11. *Rules:* Subordinado à Lei #2 (Saída Estritamente Binária).
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Não está no `.pre-commit-config.yaml`: roda sob demanda (`python gates/G_DOCS_ROT.py`).
  8. *Agents:* Executa sem intervenção de agente cognitivo.
  9. *Skills:* Invocado pelas skills de runner e auditoria.
  10. *MCPs:* Nenhum — o portão não depende de servidor MCP para rodar.
  11. *Rules:* Subordinado à Lei #2 (Saída Estritamente Binária).
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  2. *Cria / Processa:* Análise estática por AST Python, regex ou schema JSON (Zero LLM).
  3. *Entrega (Output):* Código binário rigoroso (`exit 0` = APROVADO, `exit 1` = BLOQUEADO).
  4. *Configs:* Argumentos CLI: `--strict`, `--target`
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Roda automaticamente no commit via `.pre-commit-config.yaml`.
  8. *Agents:* Executa sem intervenção de agente cognitivo.
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  2. *Cria / Processa:* Análise estática por AST Python, regex ou schema JSON (Zero LLM).
  3. *Entrega (Output):* Código binário rigoroso (`exit 0` = APROVADO, `exit 1` = BLOQUEADO).
  4. *Configs:* Argumentos CLI: `--env-file`, `--root-dir`
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Roda automaticamente no commit via `.pre-commit-config.yaml`.
  8. *Agents:* Executa sem intervenção de agente cognitivo.
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Não está no `.pre-commit-config.yaml`: roda sob demanda (`python gates/G_ESCRITOR_ATOMICO.py`).
  8. *Agents:* Executa sem intervenção de agente cognitivo.
  9. *Skills:* Invocado pelas skills de runner e auditoria.
  10. *MCPs:* Nenhum — o portão não depende de servidor MCP para rodar.
  11. *Rules:* Subordinado à Lei #2 (Saída Estritamente Binária).
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  2. *Cria / Processa:* Análise estática por AST Python, regex ou schema JSON (Zero LLM).
  3. *Entrega (Output):* Código binário rigoroso (`exit 0` = APROVADO, `exit 1` = BLOQUEADO).
  4. *Configs:* Argumentos CLI: `--check-artifact`, `--type`
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Roda automaticamente no commit via `.pre-commit-config.yaml`.
  8. *Agents:* Executa sem intervenção de agente cognitivo.
...
### 19. Guarda Incorruptível: `G_HADOLINT.py`
...
### 20. Guarda Incorruptível: `G_HANDOFF_MELHORIA.py`
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  2. *Cria / Processa:* Análise estática por AST Python, regex ou schema JSON (Zero LLM).
  3. *Entrega (Output):* Código binário rigoroso (`exit 0` = APROVADO, `exit 1` = BLOQUEADO).
  4. *Configs:* Argumentos CLI: `--handoff`, `--repo-root`
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Não está no `.pre-commit-config.yaml`: roda sob demanda (`python gates/G_HANDOFF_MELHORIA.py`).
  8. *Agents:* Executa sem intervenção de agente cognitivo.
  9. *Skills:* Invocado pelas skills de runner e auditoria.
  10. *MCPs:* Nenhum — o portão não depende de servidor MCP para rodar.
  11. *Rules:* Subordinado à Lei #2 (Saída Estritamente Binária).
...
### 21. Guarda Incorruptível: `G_HARNESS_COMPAT.py`
...
### 22. Guarda Incorruptível: `G_HONESTIDADE_ROTULO.py`
...
### 23. Guarda Incorruptível: `G_IDIOMA_LEI_4.py`
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  2. *Cria / Processa:* Análise estática por AST Python, regex ou schema JSON (Zero LLM).
  3. *Entrega (Output):* Código binário rigoroso (`exit 0` = APROVADO, `exit 1` = BLOQUEADO).
  4. *Configs:* Argumentos CLI: `--caminho`
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Roda automaticamente no commit via `.pre-commit-config.yaml`.
  8. *Agents:* Executa sem intervenção de agente cognitivo.
...
### 24. Guarda Incorruptível: `G_INFRA_COMPOSE.py`
...
### 25. Guarda Incorruptível: `G_ISOLATION_AUDIT.py`
...
### 26. Guarda Incorruptível: `G_LAYOUT_ENTREGA.py`
...
### 27. Guarda Incorruptível: `G_LEI_DECLARA_PORTAO.py`
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  2. *Cria / Processa:* Análise estática por AST Python, regex ou schema JSON (Zero LLM).
  3. *Entrega (Output):* Código binário rigoroso (`exit 0` = APROVADO, `exit 1` = BLOQUEADO).
  4. *Configs:* Argumentos CLI: `--agents-file`
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Roda automaticamente no commit via `.pre-commit-config.yaml`.
  8. *Agents:* Executa sem intervenção de agente cognitivo.
...
### 28. Guarda Incorruptível: `G_LIVRO_EVIDENCIA.py`
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  2. *Cria / Processa:* Análise estática por AST Python, regex ou schema JSON (Zero LLM).
  3. *Entrega (Output):* Código binário rigoroso (`exit 0` = APROVADO, `exit 1` = BLOQUEADO).
  4. *Configs:* Argumentos CLI: `--json`, `--livro`, `--projeto`
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Registrado no `.pre-commit-config.yaml` com `stages: [manual]` (roda só quando chamado).
  8. *Agents:* Executa sem intervenção de agente cognitivo.
  9. *Skills:* Invocado pelas skills de runner e auditoria.
  10. *MCPs:* Nenhum — o portão não depende de servidor MCP para rodar.
  11. *Rules:* Subordinado à Lei #2 (Saída Estritamente Binária).
...
### 29. Guarda Incorruptível: `G_LLM_PROMPT_SHIELD.py`
...
### 30. Guarda Incorruptível: `G_MIGRATION_ROT.py`
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  2. *Cria / Processa:* Análise estática por AST Python, regex ou schema JSON (Zero LLM).
  3. *Entrega (Output):* Código binário rigoroso (`exit 0` = APROVADO, `exit 1` = BLOQUEADO).
  4. *Configs:* Argumentos CLI: `--target`
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Roda automaticamente no commit via `.pre-commit-config.yaml`.
  8. *Agents:* Executa sem intervenção de agente cognitivo.
...
### 31. Guarda Incorruptível: `G_ORQUESTRADOR_SINCRONO.py`
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Não está no `.pre-commit-config.yaml`: roda sob demanda (`python gates/G_ORQUESTRADOR_SINCRONO.py`).
  8. *Agents:* Executa sem intervenção de agente cognitivo.
  9. *Skills:* Invocado pelas skills de runner e auditoria.
  10. *MCPs:* Nenhum — o portão não depende de servidor MCP para rodar.
  11. *Rules:* Subordinado à Lei #2 (Saída Estritamente Binária).
...
### 32. Guarda Incorruptível: `G_PACOTE_CORE.py`
...
### 33. Guarda Incorruptível: `G_PIPELINE_HANDOFF.py`
...
### 34. Guarda Incorruptível: `G_PORTAO_PROVA_QUE_MORDE.py`
...
### 35. Guarda Incorruptível: `G_PROTOCOL_FALLBACK.py`
...
### 36. Guarda Incorruptível: `G_PROTOTYPE_REWRITE.py`
...
### 37. Guarda Incorruptível: `G_QUARTETO_SINE_QUA_NON.py`
...
### 38. Guarda Incorruptível: `G_RESUMO_USUARIO.py`
...
### 39. Guarda Incorruptível: `G_SAIDA_BINARIA.py`
...
### 40. Guarda Incorruptível: `G_SEGREDOS.py`
...
### 41. Guarda Incorruptível: `G_SKILL_ROT.py`
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  2. *Cria / Processa:* Análise estática por AST Python, regex ou schema JSON (Zero LLM).
  3. *Entrega (Output):* Código binário rigoroso (`exit 0` = APROVADO, `exit 1` = BLOQUEADO).
  4. *Configs:* Argumentos CLI: `--repo-root`, `--skills-dir`
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Roda automaticamente no commit via `.pre-commit-config.yaml`.
  8. *Agents:* Executa sem intervenção de agente cognitivo.
...
### 42. Guarda Incorruptível: `G_STACK_PADRAO_OURO.py`
...
### 43. Guarda Incorruptível: `G_SUPPLY_CHAIN.py`
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Não está no `.pre-commit-config.yaml`: roda sob demanda (`python gates/G_SUPPLY_CHAIN.py`).
  8. *Agents:* Executa sem intervenção de agente cognitivo.
  9. *Skills:* Invocado pelas skills de runner e auditoria.
  10. *MCPs:* Nenhum — o portão não depende de servidor MCP para rodar.
  11. *Rules:* Subordinado à Lei #2 (Saída Estritamente Binária).
...
### 44. Guarda Incorruptível: `G_SYNC_CMD_ROT.py`
...
### 45. Guarda Incorruptível: `G_TEMPLATE_FORGE_ROT.py`
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Não está no `.pre-commit-config.yaml`: roda sob demanda (`python gates/G_TEMPLATE_FORGE_ROT.py`).
  8. *Agents:* Executa sem intervenção de agente cognitivo.
  9. *Skills:* Invocado pelas skills de runner e auditoria.
  10. *MCPs:* Nenhum — o portão não depende de servidor MCP para rodar.
  11. *Rules:* Subordinado à Lei #2 (Saída Estritamente Binária).
...
### 46. Guarda Incorruptível: `G_TESTES_REAIS.py`
...
### 47. Guarda Incorruptível: `G_TRANSACTION_LOG_LRU.py`
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Não está no `.pre-commit-config.yaml`: roda sob demanda (`python gates/G_TRANSACTION_LOG_LRU.py`).
  8. *Agents:* Executa sem intervenção de agente cognitivo.
  9. *Skills:* Invocado pelas skills de runner e auditoria.
  10. *MCPs:* Nenhum — o portão não depende de servidor MCP para rodar.
  11. *Rules:* Subordinado à Lei #2 (Saída Estritamente Binária).
...
### 48. Guarda Incorruptível: `G_UNIVERSAL_HARNESS.py`
...
### 49. Guarda Incorruptível: `G_USER_FACING_PTBR.py`
...
### 50. Guarda Incorruptível: `G_ZERO_HEADLESS.py`
...
- **As 11 Dimensões deste Quality Gate:**
  ...
  2. *Cria / Processa:* Análise estática por AST Python, regex ou schema JSON (Zero LLM).
  3. *Entrega (Output):* Código binário rigoroso (`exit 0` = APROVADO, `exit 1` = BLOQUEADO).
  4. *Configs:* Argumentos CLI: `--dangerously-force-headless`
  5. *Gates:* Autovalidado pelo meta-gate `G_PORTAO_PROVA_QUE_MORDE.py`.
  6. *Scripts 0 LLM:* 100% mecânico e matemático.
  7. *Hooks:* Roda automaticamente no commit via `.pre-commit-config.yaml`.
  8. *Agents:* Executa sem intervenção de agente cognitivo.
...
# PARTE IV: OS 141 COMPONENTES AGNÓSTICOS (PEÇAS UNIVERSAIS LEGO)
...
## Na Casa (A Sincronização dos 141 Componentes)
- **Fonte Canônica Única:** `componentes/compartilhado/`
- **Ambientes Espelhados (Harnesses):** `.agents/`, `.claude/`, `.gemini/`, `.cursor/`, `.opencode/`, `.mimocode/`, `.codebuddy/`.
...
### As 6 Famílias dos 141 Componentes:
...
4. **Habilidades Atômicas (`componentes/compartilhado/skills/`):** 68 pastas de skills padronizadas.
...
Quando você junta as **8 Ferramentas Macro**, as **68 Micro-Ferramentas**, os **50 Quality Gates** e os **141 Componentes Agnósticos**, o que você tem não é apenas um projeto de software: você tem uma **catedral digital determinística**.
...
*Fim da Auditoria Canônica — 24 de setembro de 2026.*
