...
abstract: |
  ...
  **Ecossistema AIDD**, cobrindo cada etapa desde o acesso inicial ao repositório monorepo até
  ...
  51 Quality Gates globais e o Quarteto Sine Qua Non exigido pela Lei #10).
---
...
# 3. O Alvará e a Fundação: Preparando o Terreno Antes de Sonhar com o Bolo

## Na Festa
Antes de qualquer festa acontecer, alguém precisa comprar o terreno, pedir o alvará na prefeitura e pregar as regras da casa na parede: onde fica a saída de emergência, quem tem a chave de cada cômodo, que horas o barulho tem que parar. Ninguém decora o bolo nem escreve o cardápio nessa hora — essa etapa não cozinha nada. Ela só garante que, quando a cozinha entrar em ação daqui a pouco, ninguém vai construir em cima de um terreno sem escritura.

## Na Casa
Essa etapa é a **`aidd-forge`** — a fundação e governança suprema do ecossistema. Ela nunca gera código de aplicação; ela injeta a infraestrutura de regras no projeto alvo.

- **Como acionar sozinho** (fora de um fluxo completo, só para inspecionar):
  ```bash
  python ecossistema.py forge init <pasta-do-projeto> --force
  ```
- **O que ela realmente grava no disco** (saída real do comando — código-fonte em `tools/aidd-forge/aidd_forge/cli.py:59-74`):
  - Governança: `AGENTS.md`/`CLAUDE.md`/`GEMINI.md` apontando para a mesma fonte de regras.
  - As skills espelhadas em todos os harnesses de IA instalados na máquina.
  - As fases do projeto "cercadas" (Phase Fencer), para uma fase não pisar na outra.
  - Slash commands prontos (`/pure`, `/aidd-open`, `/freedom`, etc.).
  - O hook de `pre-commit` e os Quality Gates que vão fiscalizar todo commit futuro.

Nenhum desses itens é "a casa pronta" — são a fundação, o alvará e as regras da obra. Por isso a ordem certa é sempre **forge → planner (seção 6) → motor de construção (seção 7)**, nunca o contrário: não dá para desenhar a planta baixa num terreno que ainda não existe, nem chamar um fiscal (Quality Gate) que ainda não foi instalado.

Na prática você raramente digita o comando acima à mão: quando roda `/pure`, `/aidd-open` ou `/freedom` (seção 7), o `forge init` e o `planner init` disparam sozinhos, nessa ordem, como as duas primeiras etapas do fluxo. A prova está em código, não em opinião — abra `scripts/orquestrador_sincrono.py:544-545` e leia literalmente `("Etapa 1: aidd-forge", ...)` antes de `("Etapa 2: aidd-planner", ...)`.

---

# 4. A Conversa Antes da Massa: A Entrevista que Evita Erros
...
# 5. A Receita na Geladeira: Especificação com Critérios Binários
...
# 6. A Planta do Arquiteto: O Plano Estruturado e o Grafo VSA

## Na Festa
Agora entra o *Arquiteto da Obra* — só que ele trabalha em cima do terreno que a fundação da seção 3 já deixou pronto, nunca antes. Ele pega a receita da geladeira e desenha a planta baixa da cozinha. Ele diz: "Olha, não dá para colocar a cobertura antes da massa esfriar. Então a equipe da massa trabalha primeiro. Mas a equipe que espreme as laranjas do suco pode trabalhar ao mesmo tempo em outra bancada, porque uma não atrapalha a outra".  
Ele desenha setinhas mostrando quem espera quem. Se uma setinha fizer um círculo que nunca acaba (ciclo infinito), o arquiteto apaga e desenha de novo. Repare: ele só desenha. Quem quebra ovo e liga o forno é a cozinha da seção 7.

## Na Casa
Essa etapa é operada pela ferramenta `tools/aidd-planner`, sempre depois da `aidd-forge` (seção 3) e sempre antes do motor de construção (seção 7). Ela não cria pastas nem instala governança — só desenha.

- **Assinatura real do comando** (a mesma que `scripts/orquestrador_sincrono.py:161-168` chama por baixo dos panos):
  ```bash
  python ecossistema.py planner init \
    --fluxo 1 \
    --nome "Clinica Veterinaria" \
    --slug clinica-vet \
    --dominio "agendamento-veterinario" \
    --pasta ./clinica-vet
  ```
  `--fluxo` aceita `1` (`aidd-generator`), `2` (`aidd-factory`) ou `3` (`aidd-bridge`) — a mesma numeração da Tríade da seção 7. No chat, o assistente preenche esses parâmetros sozinho a partir da entrevista (seção 4) e da especificação (seção 5): basta dizer `/planner`.
...
- **O que ele gera:**
  ...
  3. Um contrato de handoff (`HANDOFF_PLANNER_ENGINE.json`), validado contra `handoff-planner-to-engine.schema.json`, que o motor de construção da seção 7 lê para saber exatamente o que construir. Sem esse arquivo, a construção nem começa.

---

# 7. Escolhendo a Cozinha: A Tríade Canônica
...
# 8. Mãos na Massa: Salas Secretas que Nascem e Somem
...
- **Como funciona:**
  O script `tools/aidd-master/scripts/dispatch_pipeline.py` cria a worktree (o nome da branch
  segue o padrão `slice/<id-da-fatia>`, definido em `_create_slice_worktree`):
  ```bash
  git worktree add .worktrees/fatia-agendamento -b slice/fatia-agendamento
  ```
- **Garantia Inviolável:**
...
# 9. A Prova Sem Mentira: Teste Primeiro, Código Depois
...
# 10. A Barreira da Alfândega: Juntando Tudo Sem Bagunça
...
## Na Casa
Esse é o papel do motor `vsa_join_barrier.py` e dos **51 Quality Gates** globais do ecossistema.

- **A Barreira de Junção (Join Barrier):**
  Lê os arquivos alterados no git diff da worktree e compara com o escopo autorizado da fatia: só `src/slices/<fatia>/`, `tests/` e `docs/`. Se o desenvolvedor ou a IA tocou em arquivos de outro módulo ou do núcleo compartilhado sem autorização, o merge é sumariamente bloqueado.
- **A Auditoria de Portões:**
  Antes de selar o commit, o comando `python ecossistema.py audit` roda os 40 guardas ligados ao `pre-commit`. Os outros 11 só rodam quando alguém os chama (1 é manual de propósito e 10 ainda não foram ligados ao commit), então um `audit` verde não prova que eles passaram:
  - `G_TESTES_REAIS`: Roda de verdade o `pytest` de cada uma das 8 ferramentas (2.343 testes coletados em 25/09/2026).
...
# 11. O Bolo Pronto com as 4 Peças Obrigatórias
...
# 12. Ligando o Forno: Vendo o App Vivo no Navegador
...
# 13. Guardando a Receita: Salvar o Progresso sem Levar o Prato do Vizinho

## Na Festa
No fim do dia, alguém tira foto de tudo o que ficou pronto e guarda no álbum da casa. O fotógrafo é rápido: fotografa a mesa inteira de uma vez. O problema é quando dois cozinheiros dividem a mesma mesa. A foto sai com o bolo de um e a torta do outro, e a legenda só fala do bolo.

## Na Casa
O "fotógrafo" é o **`faz-commit`**. Digitado no Git Bash, ele executa sempre o `scripts/faz_commit.py` do repositório (desde 24/09/2026, o comando global é só um atalho para ele):

1. `git add -A` — coloca **tudo** o que mudou na pasta no commit.
2. Gera a mensagem a partir do diff (por IA, se houver chave de API; senão, pergunta para você).
3. Faz o commit, que dispara os guardas do `pre-commit`, cada um mostrado em **uma linha** com o andamento ao vivo (`-v` mostra tudo).
4. Faz o push.

Se um guarda reprovar, o resumo final diz a etapa, o guarda, o `arquivo:linha` e o comando exato para repetir só aquele ponto. `faz-commit --dry-run` simula tudo e devolve o stage exatamente como estava.

**Cuidado com duas sessões de IA na mesma pasta:** como o passo 1 leva tudo, o trabalho da outra sessão entra junto. Isso aconteceu de verdade em 24/09/2026 (commit `a1ea899`). Com mais de uma sessão aberta, prefira `git add <arquivos>` e `git commit` só do que é seu.

Para saber depois qual sessão fez o quê, registre cada uma com `python ecossistema.py sessao registrar` (ou `/aidd-sessao`). O ID vai para `secoes/historico_sessoes.json` e para o índice legível `secoes/INDICE-SESSOES.md`; `sessao listar` e `sessao buscar` consultam.

---

# 14. Resumo das Palavras Mágicas

Se você precisar de uma folha de cola rápida para lembrar o que dizer em cada momento — a ordem das linhas abaixo é a ordem real de execução, não é só didática:
...
| **Instalar a fundação e a governança** | *(roda sozinho dentro de `/pure`, `/aidd-open`, `/freedom`)* | `python ecossistema.py forge init <pasta> --force` |
| **Desenhar a planta de fatias** | `/planner` | `python ecossistema.py planner init --fluxo N --nome ... --pasta ...` |
...
| **Anotar a sessão** | *"Registre esta sessão"* ou `/aidd-sessao` | `python ecossistema.py sessao registrar --id <ID> --harness claude --titulo "..."` |
| **Registrar a vitória** | *"Salve o progresso"* | `faz-commit` (no Git Bash) |
