...
Coordenador fino. **Não reimplementa** lógica de autoria de skill — delega para
ferramenta nativa de autoria de skills quando o harness atual dispuser dela, e só
garante que o resultado siga a convenção física deste projeto: fonte única em
...
1. **Descubra o harness ativo e suas capacidades.** Se o harness dispuser de
   uma ferramenta/função nativa de criação/edição de skills, invoque-a para
   toda a parte de autoria/melhoria/avaliação (estrutura de frontmatter,
   clareza de `description` pra triggering correto, evitar ambiguidade com
   outras skills existentes). Não decida essas questões sozinho quando a
