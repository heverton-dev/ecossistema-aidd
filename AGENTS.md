...
- gates/G_ARQUITETURA_DELIVERABLE.py: Audita estaticamente (via AST) a conformidade com Clean Architecture/DDD dos módulos e templates gerados — bloqueia SQL/sqlite3 fora de infrastructure/, imports de infraestrutura dentro de domain/ e acoplamento direto de banco/cache em rotas. Em `stages: [manual]` (código legado ainda tem violações conhecidas, a corrigir na Fase 2 do plano de reestruturação DDD).
