...
- Any user input referencing `/freedom`, `freedom`, `/pure`, `pure`, `/open`, `/aidd-open`, `open`, `/factory`, `/bridge`, `bridge`, or natural language equivalents (e.g. "desacoplar lovable", "criar do zero puro", "migrar open source") MUST be intercepted immediately by the agent as the direct invocation of the corresponding Canonical Flow or Tool.
...
- Immediately execute the corresponding skill (`freedom`, `pure`, `aidd-open`, `open`, `aidd-bridge-runner`) or CLI command (`python ecossistema.py freedom|pure|open|bridge <args>`).
